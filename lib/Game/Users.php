...
SQL;
    ...
    public function update(User $user) {
        ...
        if ($user == null || $user->getJoined() == null || $user->getEmail() == null || $user->getName() == null || $user->getId() == null) {
            return false;
        }

        $sql =<<<SQL
UPDATE $this->tableName
SET email = ?, name = ?, joined = ?
WHERE id = ?
SQL;
        $pdo = $this->pdo();
        $statement = $pdo->prepare($sql);
        ...
        try {
            $ret = $statement->execute(array($user->getEmail(),$user->getName(),$user->getJoined(),$user->getId()));
    ...
    public function updateStrict(User $user) {
        $sql =<<<SQL
UPDATE $this->tableName
SET email = ?, name = ?, phone = ?, address = ?, notes = ?, joined = ?, role = ?
WHERE id = ?
SQL;
        $pdo = $this->pdo();
        $statement = $pdo->prepare($sql);

        $statement->execute(array($user->getEmail(),$user->getName(),$user->getPhone(),$user->getAddress(),$user->getNotes(),$user->getJoined()
...
SQL;
                ...
                $statement->execute(array($us[$i]));
                if ($statement->rowCount() === 0) {
                    return null;
                }
                array_push($ret, $statement->fetch(\PDO::FETCH_ASSOC));
            }
        }
...
SQL;
    ...
    public function add(User $user, Email $mailer) {
        ...
        $sql = <<<SQL
INSERT INTO $this->tableName(email, name, joined, validated)
values(?, ?, ?, ?)
SQL;

        $statement = $this->pdo()->prepare($sql);
        $statement->execute([
            $user->getEmail(), $user->getName(), date("Y-m-d H:i:s"), 0
        ]);
